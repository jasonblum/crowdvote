...
class UserFactory(DjangoModelFactory):
    ...
    def social_links(obj, create, extracted, **kwargs):
        ...
        if create:
            # Only set social links if they are None (not explicitly set to empty string)
            if obj.twitter_url is None and fake.boolean(chance_of_getting_true=40):
                obj.twitter_url = f"https://twitter.com/{obj.username}"
            if obj.linkedin_url is None and fake.boolean(chance_of_getting_true=30):
